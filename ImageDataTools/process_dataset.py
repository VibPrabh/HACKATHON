...
with open("../datasets/metadata.csv", "w", newline="") as f:
    ...
    if do_simple_dataset:
        ...
        for newhandpd_folder in glob("../datasets/raw/NewHandPD/*"):
            ...
            for image_path in tqdm(glob(newhandpd_folder+"/*.jpg")):
                ...
                row = [f"{classification[0]}_{str(image_counter[classification])}", "NewHandPD", "images", f"{classification}/{image_counter[classification]}_newHandPD_{classification}.png", old_image_size, new_image_size, classification]
        ...
        for newhandpd_folder in glob("../datasets/raw/Spiral_HandPD/*"):
            ...
            for image_path in tqdm(glob(newhandpd_folder+"/*.jpg")):
                ...
                row = [str(image_counter[classification]), "SpiralHandPD", "images", f"{classification}/{image_counter[classification]}_spiralHandPD_{classification}.png", old_image_size, new_image_size, classification]
                image_counter[classification] += 1
                writer.writerow(row)
        ...
        for split in ["testing", "training"]:
            for newhandpd_folder in glob(f"../datasets/raw/Kaggle_Parkinsons_Drawings/spiral/{split}/*"):
                ...
                for image_path in tqdm(glob(newhandpd_folder + "/*.png")):
                    ...
                    row = [str(image_counter[classification]), "Kaggle", "images", f"{classification}/{image_counter[classification]}_Kaggle_{classification}.png", old_image_size, new_image_size, classification]
                    image_counter[classification] += 1
                    writer.writerow(row)
    ...
    for person_path in glob("../datasets/raw/PaHaW_public/*"):
        ...
        for test_1 in glob(person_path +"/*1_1.svc"):
            ...
            row = [f"{classification[0]}_{str(image_counter[classification])}", "NewHandPD", "reconstruction", f"{classification}/{image_counter[classification]}_PaHaW_{classification}.png", "reconstructed", "(256,256)", classification]
            image_counter[classification] += 1
            writer.writerow(row)
